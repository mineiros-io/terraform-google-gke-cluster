...
locals {
  vpc_name = "terraform-google-gke-cluster-unit-minimal"
  subnet = {
    name       = local.vpc_name
    cidr_range = "10.4.0.0/20"
    secondary_ip_ranges = {
      pods = {
        name       = "terraform-google-gke-cluster-unit-minimal-pods"
        cidr_range = "10.4.128.0/17"
      }
      services = {
        name       = "terraform-google-gke-cluster-unit-minimal-services"
